...
//       addAlert(
//         "Error",
//         "Submission failed. Please try again. If error persists, please contact admin.",
//         "error"
//       );
//     }
//   };
...
const questions: {
  ...
  common: [
    ...
    },
    {
      id: "businessPhoneNumber",
      label: "Business phone number",
      // OTP verification removed - sublabel updated
      subLabel: "(Phone number for contact purposes)",
      type: "number",
      validation: validations.nigerianPhone,
    },
    {
...
const RegistrationFlow = () => {
  ...
  const [isRadioModalOpen, setIsRadioModalOpen] = useState(false);
  const [tempRadioSelection, setTempRadioSelection] = useState<string>("");
  // OTP verification states commented out
  // const [showOTPDialog, setShowOTPDialog] = useState(false);
  // const [isPhoneVerified, setIsPhoneVerified] = useState(false);
  ...
  // Phone verification parameter removed from getFilteredQuestions
  const getFilteredQuestions = () => {
    ...
    // Phone verification filter removed - businessPhoneNumber will always show
    // if (phoneVerified) {
    //   allQuestions = allQuestions.filter((q) => q.id !== "businessPhoneNumber");
    // }
  ...
  const handleNext = () => {
    ...
    // OTP verification check removed - proceed to next question normally
    // if (currentQuestion.id === "businessPhoneNumber" && !isPhoneVerified) {
    //   setShowOTPDialog(true);
    //   return;
    // }
  ...
  // OTP verification handler commented out
  // const handleOTPVerified = () => {
  //   setShowOTPDialog(false);
  //   setIsPhoneVerified(true);
  ...
  const handleSubmit = async () => {
    ...
    const submissionData = {
      ...processedFormData,
      submittedAt: new Date().toISOString(),
      // Phone verification flag removed
      // phoneVerified: isPhoneVerified,
    };
  ...
  const handleNewRegistration = () => {
    ...
    setCurrentStep(0);
    setEntityType("");
    // Phone verification reset removed
    // setIsPhoneVerified(false);
  };
